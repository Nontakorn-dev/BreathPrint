...
export function ScreeningHistory({ sessions }: ScreeningHistoryProps) {
  ...
  if (completed.length === 0) {
    return (
      <div className="surface-card text-center py-10">
        <p className="text-muted text-sm">ยังไม่มีประวัติการคัดกรอง</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-semibold text-muted uppercase tracking-wide px-1">รายการคัดกรอง</p>
      {completed.map((session) => (
        <Link key={session.id} to={`/results/${session.id}`}>
          <div className="surface-card hover:border-brand/30 transition-all cursor-pointer group p-4 sm:p-5">
            ...
            {!session.synced && (
              <p className="text-xs text-warn mt-2 font-medium">รอ sync ออฟไลน์</p>
            )}
          </div>
        </Link>
      ))}
